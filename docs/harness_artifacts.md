...
| `<task_cwd>/metrics.json` or `<session_dir>/metrics.json` | harness solver (`write_run_metrics`) | token totals, token-weighted `prompt_cache` metrics, `tokens_by_role`, wall time, guardrail counters, provenance, and resolved config | `post-run` | No. | No. | Yes. | Yes, to group runs, check cost, and check provenance. | `metrics.prompt_cache` separates observed and unobserved responses; its hit ratio stays null when the server supplies no cache counts. `metrics.tokens_by_role` charges each main or side response once to the effective role and includes request, prompt, completion, cached, and total counts. It is not ready during the run. Assistant resume replaces it. Do not use final totals or counters as detector evidence. |
