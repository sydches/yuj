...
| `<session_id>/path_attachments.json` | Bounded repository-path manifest. It binds the evidence to the original task and records each selected repository-relative path, expanded file path, raw and admitted SHA-256 digest, byte count, redaction status, value-free security findings, and session-relative saved path. It stores no absolute source path or file content. A raw digest identifies pre-redaction bytes without retaining them. |
| `<session_id>/path_attachments/files/file-NNNN.txt` | Permission-restricted admitted UTF-8 text for one repository file. Yuj writes this copy after injection scanning and secret redaction. Assistant start and resume verify it and never reopen the source path. |
| `<session_id>/github_context.json` | Permission-restricted, task-bound GitHub issue or pull-request context from an explicit `--github` input. It stores the source identity, exact admitted representation, selected-field list, byte counts, SHA-256 digests, redaction state, and value-free security findings. It records a fixed read-only boundary. Resume and fork verify this file without another GitHub request. |
| `<session_id>/review_target.json` | Dedicated review identity and read-only contract. It records the selected target kind, resolved commit or retained-session identity, complete raw and admitted hashes and sizes, shown hash and size, truncation and omission facts, redaction state, value-free scan findings, task binding, and the fixed model-tool and repository-write settings. It stores no diff content. |
| `<session_id>/review_target.patch` | Permission-restricted admitted review diff after injection scanning, secret redaction, and the documented 512 KiB model-input bound. Review start, resume, status, show, and fork verify its hash and task binding without invoking Git or reopening the original target. |
...
| `<session_id>/provider.toml` | Model-service, thinking-level, plan-mode, permission-preset, or edit-format overrides given on the `code`, `run`, `review`, or `smoke` command. Present only when that command adds one of those overrides. |
...
| `<session_id>/subagents/<id>/changeset.json` | Bounded manifest for one isolated child's final workspace difference, including paths, hashes, sizes, limits, and handoff status. |
| `<session_id>/subagents/<id>/changes.patch` | Immutable binary Git patch for one ready isolated-child change set. |
| `<session_id>/subagents/<id>/application.json` | Receipt written only after the parent applies that exact patch successfully. |
...
| `<session_id>/approval_request.json` | Current tool approval request, stable action identity, matched permission rule when applicable, status, and a bounded proposal preview. A file preview can contain current and proposed file text. Yuj generates it before the tool runs, caps it at 120 lines and 16,000 escaped characters, and never lets it enter measurement mode. Bash requests retain `cmd` for compatibility and state that a reliable file diff is unavailable. |
...
| `<run_dir>/subagents/<id>/changeset.json` and `changes.patch` | isolated-subagent runtime (`harness/subagent_workspace.py`) | Immutable manifest and bounded Git patch for one child's final difference from its exact parent snapshot. | Ready after the isolated child ends and capture succeeds. | Only through `subagent_changes` and `apply_subagent`. | No. | Yes. | No. | The manifest accepts at most 64 files and 256 KiB of before-and-after input. The patch accepts at most 512 KiB. The parent may apply only a `ready` set. Replay may copy both files. |
| `<run_dir>/subagents/<id>/application.json` | `apply_subagent` | Patch, task, and final-workspace hashes for one successful parent application. | Ready only after exact application succeeds. | No. | No. | Yes. | No. | This receipt prevents repeated application. Replay never copies it because a replay workspace needs a fresh permission and stale-state check. |
| `<run_dir>/transcripts/<task>.log`, `<run_dir>/harness_run/transcripts/<task>.log`, `<session_dir>/transcript.log`, `<session_dir>/transcript.pre_seg_N.log`, child `transcript.log`, or legacy `<task_cwd>/transcript.log` | model client transcript writer | Saved model requests and responses. | `live-prefix`; only audit, explicit resume, replay, or an explicit redacted session export may read it. | No for normal context. | No. | Yes. | No. | Assistant resume moves the preceding parent transcript to the next numbered segment. An image request includes its transport-encoded image content here; normal inspection commands do not print it. `yuj export` selects operator text and final assistant text, applies redaction, and omits images, raw requests and responses, system prompts, configuration, credentials, and reasoning. The client reconstructs streaming records as evidence; they are not exact wire bytes. A stream-rule interrupt keeps the partial response and injection needed for replay. |
...
| `<run_dir>/savings/<task>.jsonl`, `<assist_home>/sessions/<session_id>/savings.jsonl`, legacy `<task_cwd>/.savings.jsonl`; debug sidecars in the sibling `<ledger-stem>.transform_debug/` directory | savings ledger (`savings.py`) | Exact command, tool-output, context-render, and intervention before/after sizes and hashes; legacy cost records; complete changed text only in debug mode. | `live-prefix` append-only | No. | No. | Yes. | Yes, for transformation-size and cost accounting only. | Do not use it as evidence of behavior or task success. Debug sidecars can contain pre-redaction text and require raw-transcript handling. |
...
| `<session_dir>/.terminals/<terminal_id>.log` | interactive terminal manager | Raw capped pseudo-terminal bytes from one assistant process. A terminal may echo model-supplied input into this file. | `live-prefix` | No directly. Only unread bytes returned by a traced `terminal_io` call enter context. | No. | Yes. | No. | Private harness-owned evidence. It is not a process-control channel, state input, or scoring input. The output cap terminates the process. Explicit termination, lifetime expiry, and session teardown also stop it. |
...
| `subagent` | Child ID, descriptor, turns, tokens, final-text size, workspace mode, and optional change-set summary. | The child trace owns the exact result text and hash. Isolated change files own the reviewable patch. |
...
| `terminal_start`, `terminal_input`, `terminal_read`, `terminal_end` | Interactive command hash and limits; input hash and sizes; exact admitted output, cursor, status, and output hash; final reason and exit status. | `terminal_input` stores no input text. The raw log and returned output may contain program echo. State does not copy these rows. Replay uses them without starting a process. |
...
| `model_target`, `model`, `profile_name`, `base_url`, `context_size` | Effective secret-free main target. Yuj excludes API keys. |
...
Handoff text, advisor records, hook rows, background-process and interactive-terminal rows,
