...
| Guardrail warning | `guardrail_intervention`: error, security, contract, test-read, rumination, duplicate-call, and gate-grace warnings | Tool result before and after the warning append | `harness/_loop/_dispatch_tool_call.py` |
...
| Hooks and injections | `hook_intervention`, `injection / <name>` | Tool result before and after a hook replacement or appended block; empty text to a new injected message | `harness/_loop/_dispatch_tool_call.py`, `harness/loop.py`, `harness/injections.py` |
