...
`done` uses `message`; `bash_poll`/`bash_kill` use `proc_id`; and
`write_todos` uses its canonical todo array. Calls without a dedicated field
use a canonical sorted JSON argument object.
