...
from ._driver_setup import (
    ...
    scan_session_injections, scan_session_stream_rules,
...
def solve_task(
    ...
    if stream_security_findings:
        prompt_metadata = replace(
            prompt_metadata,
            security_findings=(
                *prompt_metadata.security_findings,
                *stream_security_findings,
            ),
            security_blocked=(
                prompt_metadata.security_blocked
                or stream_security_blocked
            ),
        )
    ...
    plan_transition_seen = has_plan_mode_transition(
        _load_trace_events(trace_path)
    )

    # Resolve the selected backend before pretests, model requests, or tool
    # calls. The returned value is pinned for every execution surface.
    env_fields = compute_runtime_envelope_fields(cfg, work_dir)
    from ..sandbox.policy import bind_sandbox_envelope

    cfg = bind_sandbox_envelope(cfg, env_fields)
    # The executable path is needed only to bind the in-memory Config. It is
    # host-specific and must not enter trace or session provenance.
    env_fields.pop("sandbox_backend_executable", None)
    if "cfg" in getattr(client, "__dict__", {}):
        client.cfg = cfg
    ...
    with open(trace_path, "a") as trace_file:
        ...
        if trace_path.stat().st_size == 0:
            ...
            log.info(
                "runtime_envelope: sandbox_mode=%s selected=%s resolved=%s "
                "backend=%s engaged=%s "
                "bwrap=%s preflight=%s container=%r runtime=%r digest=%r",
                env_fields["sandbox_mode"],
                env_fields.get("sandbox_selected", env_fields["sandbox_backend"]),
                env_fields.get("sandbox_resolved", env_fields["sandbox_backend"]),
                env_fields["sandbox_backend"],
        ...
        for session_num in range(start_session_num, end_session_num + 1):
            ...
            _emit_trace_event(
                ...
                sandbox_selected=env_fields.get(
                    "sandbox_selected", env_fields["sandbox_backend"]
                ),
                sandbox_resolved=env_fields.get(
                    "sandbox_resolved", env_fields["sandbox_backend"]
                ),
                sandbox_engaged=env_fields["sandbox_engaged"],
                sandbox_explicit_unsandboxed=env_fields.get(
                    "sandbox_explicit_unsandboxed", False
                ),
