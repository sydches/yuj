...
Snapshots use checked task reads and a private Git store. The turn→sha map
lives in the telemetry directory beside the trace, outside the model's world.
The task repository's objects, configuration and index remain unchanged.
...
from .snapshot_files import (
    capture_native_snapshot, capture_local_snapshot, snapshot_object_store,
    legacy_container_snapshot_files,
)
...
def ensure_snapshot_setup(repo_dir: Path, session=None) -> None:
    """Compatibility hook: private snapshot refs now keep objects alive."""
...
def _snapshot(repo_dir: Path, turn: int, session=None) -> str | None:
    ...
    index and objects stay in harness-owned storage for both task views.
    """
    ...
    try:
        ...
        if files is not None:
            ...
            sha = capture_local_snapshot(repo_dir, turn,
                owned_paths=_snapshot_owned_paths(repo_dir, session))
