...
def test_source_resource_contract_and_package_owned_data_load():
    ...
    assert report.package_resource_count == 15
...
def test_yuj_dry_run_stops_before_network_and_creates_no_session(
    ...
    assert "Runtime resources: source-checkout (65 root, 15 package)" in output
    assert "Runner: generic" in output
...
def test_preflight_accepts_disabled_optional_bash_layers(tmp_path):
    ...
    assert report.detected_runner == "generic"
